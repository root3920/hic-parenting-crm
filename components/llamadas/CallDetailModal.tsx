...
import { Video, Mail, Phone, Calendar, User, Tag, ExternalLink, ClipboardCheck, ChevronDown, Check, Loader2, FileCheck, Link2 } from 'lucide-react'
...
const CHECKLIST_SECTION_LABELS: Record<string, string> = {
  ...
  'child-desensitization': '2. Desensitization to Reactivity',
  'child-poking': '3. \u201CPoking the Bear\u201D Behaviors',
  'child-scanning': '4. Emotional Scanning Behaviors',
  'child-shutdown': '5. Shutdown & Emotional Disconnection',
  'child-impact': '6. Emotional Impact on the Child',
  'co-micro': '1. Micromanaging & Policing Dynamic',
  'co-undermined': '2. Feeling Undermined / \u201CNever Good Enough\u201D',
...
export function CallDetailModal({ call, onClose, onStatusChange }: Props) {
  ...
  useEffect(() => {
    if (call) {
      ...
      // Fetch checklist submission for this call's email
      if (call.email) {
        setChecklistLoading(true)
        fetch(`/api/checklist/by-call?email=${encodeURIComponent(call.email)}`)
          .then((r) => r.json())
          .then((data) => setChecklist(data.submission ?? null))
        ...
        <div className="space-y-5 mt-2">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              <span className="text-xs text-zinc-500 dark:text-zinc-400 shrink-0">Change status:</span>
            ...
            {pendingStatus && (
              <div className="flex items-center justify-between gap-3 rounded-lg bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 px-3 py-2">
                <span className="text-xs text-zinc-600 dark:text-zinc-400">
                  Change to <strong className="text-zinc-800 dark:text-zinc-200">{pendingStatus}</strong>?
                </span>
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  >
                    {updating ? <Loader2 className="h-3 w-3 animate-spin" /> : 'Confirm'}
                  </button>
                </div>
              </div>
            )}
          ...
          {/* Contact info */}
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-zinc-400 dark:text-zinc-500 mb-3">Contact</p>
          ...
          {/* Call details */}
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-zinc-400 dark:text-zinc-500 mb-3">Details</p>
            <div className="grid grid-cols-2 gap-3">
              <InfoRow icon={User} label="Closer" value={call.closer_name} />
              <InfoRow icon={User} label="Setter" value={call.setter_name} />
              <InfoRow icon={Calendar} label="Calendar" value={call.calendar} />
              <InfoRow icon={Tag} label="Activity type" value={call.activity_type} />
            </div>
          </div>
          ...
          {(call.utm_source || call.utm_campaign) && (
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-zinc-400 dark:text-zinc-500 mb-3">Origin</p>
              <div className="grid grid-cols-2 gap-3">
          ...
          {call.notes && (
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-zinc-400 dark:text-zinc-500 mb-2">Notes</p>
          ...
          {call.objections && (
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-zinc-400 dark:text-zinc-500 mb-2">Objections</p>
          ...
          {call.call_summary && (
            <div className="border-l-4 border-l-teal-400 pl-3">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-xs font-semibold uppercase tracking-wide text-teal-600 dark:text-teal-400">Closer report</p>
              </div>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                {call.call_status && (
                  <span className={cn('inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium', CALL_STATUS_STYLES[call.call_status] ?? 'bg-zinc-100 text-zinc-600')}>
                    Reported status: {call.call_status}
                  </span>
                )}
                {call.next_step && (
              ...
              {call.reported_by && call.reported_at && (
                <p className="text-xs text-zinc-400 dark:text-zinc-500 mt-1.5">
                  Reported by <span className="font-medium text-zinc-600 dark:text-zinc-300">{call.reported_by}</span> · {formatReportedAt(call.reported_at)}
                </p>
              )}
            </div>
          )}
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-xs font-semibold uppercase tracking-wide text-orange-600 dark:text-orange-400">Reactivity Checklist</p>
            </div>
          ...
          <div className="text-xs text-zinc-400 dark:text-zinc-500 pt-2 border-t border-zinc-100 dark:border-zinc-800">
            Created: {new Date(call.created_at).toLocaleDateString('en-US', { day: '2-digit', month: 'short', year: 'numeric' })}
          </div>
        </div>
      </DialogContent>
    </Dialog>
